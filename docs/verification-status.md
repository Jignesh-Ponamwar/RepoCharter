...
| npm registry/package identity | `npm view repo-charter@0.1.5 version --prefer-online` returned `0.1.5` |
| public npm package workflow | From a clean directory outside the source checkout, both `npx --yes repo-charter@0.1.5 init . --primary-agent codex --json` and latest `npx --yes repo-charter init . --primary-agent codex --json` created handoff-ready sessions with `packageVersion: "0.1.5"`. Earlier `repo-charter@0.1.3` installation with `--ignore-scripts`, installed `.cmd` help, `init`, `workflow preview`, approved `workflow apply`, and `check` passed for new and existing Git fixtures in both `local-planning` and `shared-planning` |
